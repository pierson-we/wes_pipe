...
class bqsr(luigi.Task):
	...
	def run(self):
		pipeline_utils.confirm_path(self.output()[2].path)
		cmd = [self.gatk4_location, '--java-options', '"-Xmx8g"', 'BaseRecalibrator', '-R', self.fasta_file, '-I', self.input()[0].path, '--known-sites', self.dbsnp, '--known-sites', self.mills, '--known-sites', self.kg, '-O',  self.output()[2].path]
