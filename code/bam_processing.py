...
class bowtie(luigi.Task):
	...
	def run(self):
		...
		cmd = [os.path.join(cwd, self.bowtie_location, 'bowtie2'), '-x', self.base_name, '--threads=%s' % self.max_threads, '-1', self.fastq_file[0], '-2', self.fasta_file[1], '-S', self.sample + '_raw.sam']
		...
		try:
			for sample in os.listdir(self.sample_dir):
				if os.path.isdir(os.path.join(self.sample_dir, sample)):
					...
					print(os.listdir(os.path.join(self.sample_dir, sample, 'tumor'))[0])
