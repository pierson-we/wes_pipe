...
class genome_index(luigi.Task):
	...
	def run(self):
		...
		cmd = ['bowtie2-build', '--threads=%s' % self.max_threads, self.fasta_file, self.base_name]
...
class realigner_target(luigi.Task):
	...
	def output(self):
		return [self.input()[1], luigi.LocalTarget(os.path.join(self.project_dir, 'output', self.sample[:-2], 'alignment', self.sample + '_forIndelRealigner.intervals'))]
		...
		try:
			for sample in os.listdir(self.sample_dir):
				if os.path.isdir(os.path.join(self.sample_dir, sample)):
					...
					tumor_fastq = os.path.join(self.sample_dir, sample, 'tumor', os.listdir(os.path.join(self.sample_dir, sample, 'tumor'))[0])
					sample_dict[sample]['T'] = tumor_fastq
					if os.path.exists(os.path.join(self.sample_dir, sample, 'normal')):
						normal_fastq = os.path.join(self.sample_dir, sample, 'normal', os.listdir(os.path.join(self.sample_dir, sample, 'normal'))[0])
