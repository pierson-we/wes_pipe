...
class recalibrated_bam(luigi.Task):
	...
	def output(self):
		return [luigi.LocalTarget(os.path.join(self.project_dir, 'output', self.sample[:-2], 'alignment', self.sample + '_recalibrated.bam')), luigi.LocalTarget(os.path.join(self.project_dir, 'output', self.sample[:-2], 'alignment', self.sample + '_recalibrated.bai'))]

	def run(self):
		for output in self.output():
			pipeline_utils.confirm_path(output.path)
		cmd = [self.gatk4_location, '--java-options', '"-Xmx8g -XX:+UseSerialGC"', 'ApplyBQSR', '-R', self.fasta_file, '-I', self.input()[0].path, '--bqsr-recal-file', self.input()[2].path, '-O',  self.output()[0].path]
...
class cases(luigi.Task):
	...
	def requires(self):
		...
		return [variant_analysis.vep(case=case, tumor=self.sample_dict[case]['T'], matched_n=self.sample_dict[case]['N'], project_dir=self.project_dir, max_threads=self.sample_threads, case_dict=self.sample_dict) for case in self.sample_dict] + [cnvkit(case_dict=self.sample_dict, project_dir=self.project_dir, max_threads=self.sample_threads)]
