...
class bowtie(luigi.Task):
	...
	def run(self):
		...
		cmd = [os.path.join(self.bowtie_location, 'bowtie2'), '-p', self.max_threads, '-x', os.path.join(self.fasta_dr, 'index', self.base_name), '-1', self.fastq_file.split('\t')[0], '-2', self.fastq_file.split('\t')[1], '|', os.path.join(global_vars.cwd, self.samtools_location), 'view', '-b', '-', '>', self.output().path]
