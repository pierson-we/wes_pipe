...
class freebayes(luigi.Task):
	...
	def run(self):
		pipeline_utils.confirm_path(self.output().path)
		if self.matched_n:
			cmd = ['/bin/bash', './packages/freebayes/scripts/freebayes-parallel', '''<(python3 ./packages/freebayes/scripts/fasta_generate_regions.py''', '%s.fai' % self.fasta_file, '''100000)''', self.max_threads, '-f', self.fasta_file, '-t', self.library_bed, '--pooled-continuous', '--pooled-discrete', '-F', '0.01', '-C', '2', self.input()[0][0].path, self.input()[1][0].path, '>', os.path.join(self.vcf_path, 'freebayes.vcf')]
		else:
			cmd = ['/bin/bash', './packages/freebayes/scripts/freebayes-parallel', '''<(python3 ./packages/freebayes/scripts/fasta_generate_regions.py''', '%s.fai' % self.fasta_file, '''100000)''', self.max_threads, '-f', self.fasta_file, '-t', self.library_bed, '--pooled-continuous', '--pooled-discrete', '-F', '0.01', '-C', '2', self.input()[0][0].path, '>', os.path.join(self.vcf_path, 'freebayes.vcf')]
