...
class vep(luigi.Task):
	...
	def requires(self):
		...
		return fpfilter(case=self.case, tumor=self.sample_dict[case]['T'], matched_n=self.sample_dict[case]['N'], project_dir=self.project_dir, max_threads=self.sample_threads, case_dict=self.sample_dict)
