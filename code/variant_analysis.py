...
class cases(luigi.Task):
	...
	def requires(self):
		...
		[variant_calling.msisensor(max_threads=self.max_threads, project_dir=self.project_dir, case=case, tumor=self.sample_dict[case]['T'], matched_n=self.sample_dict[case]['N'], case_dict=self.sample_dict, cfg=self.cfg, vcf_path=os.path.join(self.project_dir, 'output', case, 'variants')) for case in self.sample_dict]
