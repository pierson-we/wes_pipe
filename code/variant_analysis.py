...
class cases(luigi.Task):
	...
	def requires(self):
		...
		[variant_calling.msisensor(max_threads=self.max_threads, project_dir=self.project_dir, case=case, tumor=self.case_dict[case]['T'], matched_n=self.case_dict[case]['N'], case_dict=self.case_dict, cfg=self.cfg, vcf_path=os.path.join(self.project_dir, 'output', case, 'variants')) for case in self.case_dict]
