...
class msings_baseline(luigi.Task):
	...
	def run(self):
		...
		cmd = ['./packages/msings/scripts/create_baseline.sh', normal_bams_file, './packages/MANTIS/b37_1000_ms_loci_annotated.msi_intervals', './packages/MANTIS/b37_1000_ms_loci_annotated.bed', self.cfg['fasta_file'], os.path.join(self.project_dir, 'output', 'msings', 'baseline')]
		pipeline_utils.command_call(cmd, self.output())
...
class msi(luigi.Task):
	...
	def run(self):
		for output in self.output():
			pipeline_utils.confirm_path(output.path)
		if self.matched_n != '':
			cmd = ['python3', './packages/MANTIS/mantis.py', '-b', './packages/MANTIS/b37_1000_ms_loci_annotated.bed', '--genome', self.cfg['fasta_file'], '-t', self.input()[0][0].path, '-n', self.input()[1][0].path, '-mrq', '20.0', '-mlq', '25.0', '-mlc', '20', '-mrr', '1', '-o', self.output()[0].path]
			pipeline_utils.command_call(cmd, self.output())
		# else:
		...
		cmd = ['./packages/msings/scripts/run_msings_single_sample.sh', self.input()[0][0].path, './packages/MANTIS/b37_1000_ms_loci_annotated.msi_intervals', './packages/MANTIS/b37_1000_ms_loci_annotated.bed', self.cfg['fasta_file'], os.path.join(self.project_dir, 'output', 'msings', 'baseline', 'MSI_BASELINE.txt'), os.path.join(self.project_dir, 'output', 'msings', 'tumor')]
		pipeline_utils.command_call(cmd, self.output())
